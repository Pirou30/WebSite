<?php
/*---------------------------------------------------------------------------------------------------
                          function to register a secondary user
---------------------------------------------------------------------------------------------------*/
...
//Date variables
  ...
  $req3 -> execute(array(
   ...
   do
   {
     ...
     {
       $req = $db->prepare('INSERT INTO posseder(droit_d_utilisateur, date_de_modification_des_droits, id_utilisateur, id_piece) VALUES(:droit_d_utilisateur, :date_de_modification_des_droits, :id_utilisateur, :id_piece)');
       $req->execute(array(
...
/*---------------------------------------------------------------------------------------------------
function to display each "piece" owned by the current primary user
---------------------------------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------------------------------
                          function to get the user password from id
---------------------------------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------------------------------
                          function to get the user id from login
---------------------------------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------------------------------
          function to get the user profile infos - returning them in $_POST values
---------------------------------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------------------------------
    function to get an array containing secondary users owned by the connected primary user
---------------------------------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------------------------------
      function to get an array containing the secondary user rights for each room id
---------------------------------------------------------------------------------------------------*/
function get_primary_user_piece_rights($db, $user_id)
{
  $sql = "SELECT droit_d_utilisateur, id_piece FROM posseder WHERE id_utilisateur = '".$user_id."'";
  $request = $db->query ($sql);
...
/*---------------------------------------------------------------------------------------------------
function to modify the user profile infos
---------------------------------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------------------------------
    function to edit the user rights for a specific room
---------------------------------------------------------------------------------------------------*/
function edit_secondary_users_rights($edit, $user_id, $id_piece, $db)
{
  $today = date("Y-m-d H:i:s");
  $sql = "UPDATE posseder SET droit_d_utilisateur = '$edit', date_de_modification_des_droits = '" . $today ."' WHERE id_utilisateur = '" .$user_id. "' AND id_piece = '" .$id_piece. "'";
  $request = $db->query($sql);
}

/*---------------------------------------------------------------------------------------------------
    function to set $_SESSION with all infos of the conneced user
---------------------------------------------------------------------------------------------------*/
...
//to delete the primary account:
  if ($user_id == $connected_user)
  {
    ...
    $request = $db -> query($sql);
    //Delete all secondary users owned by the main user
    $sql = "DELETE FROM utilisateur WHERE id_utilisateur_1 = '".$user_id."'" ;
    $request = $db -> query($sql);
    //Delete all rights for primary and seconday users

    //suppresion des pièces reliées à l'utilisateur
    $sql = "DELETE FROM piece WHERE id_piece IN (SELECT id_piece FROM posseder WHERE id_utilisateur = '".$id_utilisateur."')" ;
    $request = $db -> exec($sql);

    //suppresion des objets reliés à l'utilisateur de chaque pièce
    $sql = "DELETE FROM objet_connecte WHERE id_piece IN (SELECT id_piece FROM posseder WHERE id_utilisateur = '".$id_utilisateur."')" ;
    $request = $db -> exec($sql);
    ...
    do
    {
      ...
      $sql = "DELETE FROM posseder WHERE id_utilisateur = '".$secondary_user_id."'" ;
      $request = $db -> query($sql);
      $line++;
  ...
  else
  {
    ...
    $sql = "DELETE FROM posseder WHERE id_utilisateur = '".$user_id."'" ;
    $request = $db -> query($sql);
